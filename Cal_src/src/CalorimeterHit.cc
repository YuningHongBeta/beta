...
CalorimeterHit::CalorimeterHit(G4int cellID)
: G4VHit(), 
...
const CalorimeterHit& CalorimeterHit::operator=(const CalorimeterHit
                                                          &right)
{
  fCellID = right.fCellID;
  fEdep = right.fEdep;
...
{
  G4bool isNew;
  auto store = G4AttDefStore::GetInstance("CalorimeterHit",isNew);
