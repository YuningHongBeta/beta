...
submit_one() {
  ...
  local bgo_offset_cm=$8
  local pc_front_cm=$9
  local pc_outer_deg=${10}
  ...
  bsub -J "${tag}" -n 2 -R "span[hosts=1]" -q s -W 120 \
    ...
    env \
      ...
      BETA_PC_SQUARE_HOLE_MM="${hole_mm}" \
...
for task_hole in 100 150; do
  for task_primary in e pim pi0; do
    submit_one "${task_hole}" current "${task_primary}" \
      "bgoc_balanced_pc3_h${task_hole}_s7232026_${task_primary}" \
      15 15 uniform_theta 0 55.5 24
    submit_one "${task_hole}" bgoegg_frustum "${task_primary}" \
      "bgoegg31_balanced_pc3_h${task_hole}_s7232026_${task_primary}" \
